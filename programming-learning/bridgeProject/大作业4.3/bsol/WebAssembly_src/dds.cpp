...
// Modified by John Goacher, July 2015. New function CalcDDtableAndLeads added, in order to return makeable tricks for each lead card along with makeable contracts.
// CalcDDtable modified to call this function (to avoid repeating the same code).

int STDCALL CalcDDtableAndLeads(struct ddTableDeal tableDeal, int solutions[],struct ddTableResults * tablep, struct solvedBoards * solved)
{
...
int STDCALL CalcDDtable(struct ddTableDeal tableDeal, struct ddTableResults * tablep) {
	...
	return CalcDDtableAndLeads(tableDeal,solutions,tablep,&solved);
}
...
// Modified by John Goacher, July 2015. Added function CalcDDTableAndLeadsPBN
int STDCALL CalcDDtableAndLeadsPBN(struct ddTableDealPBN tableDealPBN, int solutions[], struct ddTableResults * tablep, struct solvedBoards * solved)
{
  ...
  res=CalcDDtableAndLeads(tableDeal, solutions, tablep, solved);
...
//					throw std::runtime_error("j not in [0..3] in Par");
						// exit statement added by John Goacher June 2015 to replace the "throw" statement above
					exit(-1);
				}
