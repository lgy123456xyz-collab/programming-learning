...
// Modified by John Goacher, July 2015. Added function CalcDDTableAndLeads.
EXTERN_C DLLEXPORT int STDCALL CalcDDtableAndLeads(struct ddTableDeal tableDeal, int solutions[], struct ddTableResults * tablep, struct solvedBoards * solved);
EXTERN_C DLLEXPORT int STDCALL CalcDDtableAndLeadsPBN(struct ddTableDealPBN tableDealPBN, int solutions[], struct ddTableResults * tablep, struct solvedBoards * solved);
...
/* Added by John Goacher Jan 2014. Externalise existing routine that calculates Par Results from supplied DDTable */
EXTERN_C int Par(struct ddTableResults * tablep, struct parResults *presp, int vulnerable);
