...
from matrix_decompositions import EigenWorker
...
class MainWindow(QMainWindow):
    ...
    def do_dec(self, mode):
        ...
        matrix = self.matrix_store[a_name]

        # 针对 EIGEN 开启异步处理
        if mode == 'EIGEN':
            self.run_async_eigen(matrix)
        else:
            # 其他较快的分解仍可保持同步，但增加光标反馈
            QApplication.setOverrideCursor(Qt.CursorShape.WaitCursor)
            try:
                self.current_raw_res = decomp.decompose(mode, matrix)
                self.refresh_display()
            finally:
                QApplication.restoreOverrideCursor()

    def run_async_eigen(self, matrix):
        """异步特征分解逻辑"""
        # 强制设置光标，增强视觉反馈
        QApplication.setOverrideCursor(Qt.CursorShape.WaitCursor)
        ...
        self.loading_dialog.setWindowTitle("计算中")
        self.loading_dialog.setText("正在进行特征值分解，符号运算可能较慢，请稍候...")
        # 移除关闭按钮，防止用户手动关闭导致逻辑混乱
        self.loading_dialog.setWindowFlags(self.loading_dialog.windowFlags() | Qt.WindowType.CustomizeWindowHint)
        self.loading_dialog.setWindowFlag(Qt.WindowType.WindowCloseButtonHint, False) 
        ...
        # 实例化工作线程
        self.thread = EigenWorker(matrix)
        
        # 关键：连接信号
        self.thread.finished.connect(self.on_eigen_success)
        self.thread.error.connect(self.on_eigen_error)
        
        # 启动线程
        self.thread.start()

    def on_eigen_success(self, res):
        """成功回调"""
        # 1. 立即尝试关闭对话框
        self._cleanup_loading_ui()
        
        # 2. 更新数据并刷新显示
        self.current_raw_res = res
        self.refresh_display()
        
        # QMessageBox.information(self, "完成", "特征值分解计算完毕")

    def on_eigen_error(self, err_msg):
        """失败回调"""
        self._cleanup_loading_ui()
        QMessageBox.critical(self, "错误", f"特征分解失败: {err_msg}")

    def _cleanup_loading_ui(self):
        """清理 UI 状态的辅助函数"""
        # 恢复光标
        QApplication.restoreOverrideCursor()
        # 安全关闭对话框
        if hasattr(self, 'loading_dialog') and self.loading_dialog:
            self.loading_dialog.accept() # 使用 accept() 往往比 close() 在信号处理中更稳健
            self.loading_dialog.deleteLater() # 彻底释放内存
    ...
    def save_result(self):
        ...
        if isinstance(self.current_raw_res, dict):
            # --- 核心改进：只保存类型为 sp.Matrix 的内容 ---
            # 这样字典里的 "类型": "Jordan 分解" 这种字符串会被自动跳过
            matrices_to_save = [
                (k, v) for k, v in self.current_raw_res.items() 
                if isinstance(v, sp.Matrix)
            ]
            
            for i, (key, mat) in enumerate(matrices_to_save):
                # 如果用户提供的名字多，按顺序给；名字少，则用 名字_Key
                if i < len(raw_names):
                    save_name = raw_names[i]
                else:
                    save_name = f"{raw_names[0]}_{key}" if raw_names else f"ANS_{key}"
                
                self._update_store(save_name, mat)
                
            QMessageBox.information(self, "成功", f"已保存 {len(matrices_to_save)} 个结果矩阵")
